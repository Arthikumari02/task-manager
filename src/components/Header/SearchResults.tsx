...
import { useFetchBoardMeta } from "../../hooks/APIs/useFetchBoardMeta";
...
const SearchResults: React.FC = observer(() => {
  ...
  const { fetchBoardMeta } = useFetchBoardMeta();

  useEffect(() => {
    searchCards.forEach((card) => {
      if (card.boardId) {
        fetchBoardMeta(card.boardId);
      }
    });
  }, [searchCards]);
    ...
    <div className="p-4">
      ...
      <div className="space-y-2">
        {searchCards.map((card) => {
          const board = boardsStore.getBoardById(card.boardId);
          return (
            <div
              key={card.id}
